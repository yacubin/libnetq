...
static int deletePostRequest(NQWebRequest* request, NQWebResponse* response)
{
  ...
  if (token == NULL || !NQCStrStartsWith(token, BEARER_PREFIX)) {
    NQWebResponse_writeJsonRpcErrorParams(response, NQ_HTTP_UNAUTHORIZED, "Missing or malformed Authorization header");
  ...
  if (claims.exp < NQGetTimeSec()) {
    NQWebResponse_writeJsonRpcErrorParams(response, NQ_HTTP_UNAUTHORIZED, "Authentication token expired");
    return NQ_HTTP_UNAUTHORIZED;
  }
